...
public interface IStorefrontService {
     ...
     * 
     * To add and remove categories, assign/unassig category names directly to
     * products.
     */
    public SearchResult<Category> getCategories();

    /**
     * Gets products matching the specified criteria. The categories and reviews
     * of each product is not populated; to fetch this data, invoke the
     * {@link #getProduct(int)} method.
     * 
     * @param matchText
     *            Text that must appear in the name and/or description of the
     *            product (case insensitive). Ignored if null or empty. Leading
     *            and trailing space is ignored.
     * @param categories
     *            When non-null and non-empty, excludes products not in any of
     *            the categories specified (case sensitive).
     * @param page
     *            When non-null, specifies the max number of items to return. If
     *            only a subset of products is returned, you can still determine
     *            the total number of products matching the criteria by using
     *            the {@link SearchResult#getTotalCount()} property.
     * @param pageSize
     *            When non-null, indicates the first item to return from the
     *            resultset (offset is page * pageSize), so the page parameter
     *            is also required.
     * @param sort
     *            The order in which items should be returned. Leave null for an
     *            undefined ordering.
     * @return The matching products (on the specified page and/or to the
     *         specified limit, if specified) and total count of products
    ...
    /**
     * Gets information on a specific product, including the reviews associated
     * with it (and the customers associated with each review).
     * 
     * @param productId
     *            The product's ID.
     * @return The product
     ...
     * @param emailAddress
     *            The email address of the customer. If specified, the Customer
     *            information is updated to include this email address.
     * @param rating
    ...
    /**
     * Gets a customer with the given ID, creating the customer if no customer
     * yet exists with that ID. Collections within this object (cart contents
     * and transactions) are not fetched.
     * 
     * If a new customer is created, the specified customerId is ignored and a
     * new one is auto-generated instead.
     * 
     * If the customer already exists, that customer's dateLastActive property
    ...
    /**
     * Gets the contents of a customer's cart. The details of each product
     * (reviews, etc.) are not fetched.
     * 
     * @param customerId
    ...
    /**
     * Adds (or removes) a product in the given quantity to the customer's
     * shopping cart. If the product already exists in the shopping cart, the
     * quantity is changed to the specified value, not incremented.
     * 
     * @param customerId
     ...
     * @param quantity
     *            The quantity to add. Use 0 to remove the product from the cart
     *            entirely.
     * @return The total number of items in the cart; this accounts for products
    ...
    /**
     * Converts the contents of a customer's cart to a transaction, and clears
     * the cart.
     * 
     * @param customerId
     *            The ID of the customer completing the transaction.
     * @return The transaction object describing the transaction, including a
